...
`pay` transfers ERC-20 tokens or native ETH/gas tokens from your x402 wallet to a recipient address. It reads configuration from `~/.x402/config.toml` and handles wallet decryption, balance checking, and transaction confirmation.
...
The tool reads from `~/.x402/config.toml`. Key settings:
